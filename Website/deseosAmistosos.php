...
if (isset($_POST['nachricht']) && $cookie_id != CONFIG_DEMO_USER) {
	...
	if (mysql_num_rows($sql2) > 0) {
		...
		if ($chatSperreBis > 0 && $chatSperreBis > time()) {
			addInfoBox('Du bist noch bis zum %1$s Uhr für die Kommunikation im Spiel gesperrt. Wenn Dir unklar ist warum, frage bitte das %2$s.', date('d.m.Y H:i', $chatSperreBis), '<a class="inText" href="/quien_esta_en_linea.php">'._('Support-Team').'</a>');
			include 'zz3.php';
			exit;
		}
	}
...
<form action="/deseosAmistosos.php" method="post" accept-charset="utf-8">
...
while ($sql3 = mysql_fetch_assoc($sql2)) {
	...
	if ($sql3['user'] == $cookie_id OR $_SESSION['status'] == 'Helfer' OR $_SESSION['status'] == 'Admin') {
		echo ' <a href="/deseosAmistosos.php?delEntry='.$sql3['id'].'">'._('[Löschen]').'</a>';
	}
