...
$ligaGetValue = 'all';
$teamGetValue = '';
$ligaNav = '<p style="text-align:right"><a href="/lig_transferencias.php?liga=own" class="pagenava">'._('In eigene Liga').'</a></p>';
...
if (isset($_GET['team'])) {
	...
	if ($teamGetValue != '') {
		...
		if (mysql_num_rows($getTeamName2) > 0) {
			...
			$ligaNav = '<p style="text-align:right"><a href="/equipo.php?id='.$teamGetValue.'" class="pagenava">'._('Zum Teamprofil').'</a></p>';
		}
	}
}
if (isset($_GET['liga'])) {
	if ($_GET['liga'] == 'own') {
		...
		$ligaNav = '<p style="text-align:right"><a href="/lig_transferencias.php" class="pagenava">'._('In alle Ligen').'</a></p>';
	}
}
...
while ($sql3 = mysql_fetch_assoc($sql2)) {
	...
	echo '<td class="link"><a href="/jugador.php?id='.$sql3['spieler'].'" title="'.date('d.m.Y H:i', $sql3['datum']).' Uhr">'.mb_substr($sql3['vorname'], 0, 1, 'UTF-8').'. '.$sql3['nachname'].'</a></td>';
	echo '<td>';
	if ($sql3['gebot'] == 1) {
	...
	if (strlen($sql3['besitzer']) == 32) {
		echo '<td class="link"><a href="/equipo.php?id='.$sql3['besitzer'].'">'.ids_to_teamname($sql3['besitzer']).'</a></td>';
	}
	else {
		echo '<td>'.ids_to_teamname($sql3['besitzer']).'</td>';	
	}
	if (strlen($sql3['bieter']) == 32) {
		echo '<td class="link"><a href="/equipo.php?id='.$sql3['bieter'].'">'.ids_to_teamname($sql3['bieter']).'</a></td>';
	}
	else {
