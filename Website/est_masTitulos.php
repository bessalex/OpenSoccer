...
$standardLink = '<a href="/est_masTitulos.php?liga='.$temp_liga.'&amp;typ=';
...
while ($sql3 = mysql_fetch_assoc($sql2)) {
	...
	echo '<td>'.$counter.'.</td><td class="link"><a href="/manager.php?id='.$sql3['ids'].'">'.$sql3['username'].'</a></td><td class="link"><a href="/equipo.php?id='.$sql3['teamID'].'">'.$sql3['name'].'</a></td><td>'.$sql3['titelGesamt'].'</td><td>'.$sql3['cupsiege'].'</td><td>'.$sql3['meisterschaften'].'</td><td>'.$sql3['pokalsiege'].'</td>';
	echo '</tr>';
	$counter++;
}
?>
