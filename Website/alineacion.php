...
foreach ($spieltypenMoeglich as $spieltypSingle) {
	echo '<a href="/alineacion.php?spieltypAufstellung='.$spieltypSingle.'" class="pagenava'; if ($spieltypAufstellung == $spieltypSingle) { echo ' aktiv'; } echo '">'.$spieltypSingle.'</a>&nbsp;';
}
echo '</p>';
// SPIELTYP ENDE
?>
...
?>
<form action="/alineacion.php?spieltypAufstellung=<?php echo $spieltypAufstellung; ?>" method="post" accept-charset="utf-8">
...
while ($sql3 = mysql_fetch_assoc($sql2)) {
	...
	echo '<td'.$farbcode.'>&nbsp;</td><td>'.$mark1.$sql3['position'].$mark2.'</td><td class="link">'.$mark1.'<a href="/jugador.php?id='.$sql3['ids'].'">'.$sql3['vorname'].' '.$sql3['nachname'].'</a>'.$mark2.'</td><td>'.$mark1.floor($sql3['wiealt']/365).$mark2.'</td><td>'.$mark1.number_format($sql3['staerke'], 1, ',', '.').' <span style="color:#999">('.number_format($schaetzungVomScout, 1, ',', '.').')</span>'.$mark2.'</td><td>'.$mark1;
