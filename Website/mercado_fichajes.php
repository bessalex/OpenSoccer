...
// FILTER ENDE
?>
<form action="/mercado_fichajes.php" method="get" accept-charset="utf-8">
...
while ($sql3 = mysql_fetch_assoc($sql2)) {
    if ($sql3['bieter_highest'] != 'keiner') {
        ...
        if ($move2 !== FALSE) {
            ...
            $formulierung = __('Du hast den Spieler %1$s fur %2$s € gekauft.', '<a href="/jugador.php?id='.$sql3['spieler'].'">'.$getmanager4.'</a>', number_format($sql3['betrag_highest'], 0, ',', '.'));
            ...
            $formulierung = __('Du hast den Spieler %1$s fur %2$s € verkauft.', '<a href="/jugador.php?id='.$sql3['spieler'].'">'.$getmanager4.'</a>', number_format($sql3['betrag_highest'], 0, ',', '.'));
...
if (mysql_num_rows($sql2) == 0) {
    ...
    while ($sql3 = mysql_fetch_assoc($sql2)) {
		echo '<tr><td>'.$sql3['position'].'</td><td class="link"><a href="/subasta_mercado.php?id='.$sql3['spieler'].'">'.$sql3['vorname'].' '.$sql3['nachname'].'</a></td><td>'.floor($sql3['wiealt']/365).'</td><td>'.number_format($sql3['staerke'], 1, ',', '.');
