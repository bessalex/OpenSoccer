...
while ($sql3 = mysql_fetch_assoc($sql2)) {
	...
	while ($spieler_team1c = mysql_fetch_assoc($spieler_team1b)) {
		...
		$templist .= '<a href="/jugador.php?id='.$spieler_team1c['ids'].'">'.$temp.'</a>, ';
	...
	while ($spieler_team2c = mysql_fetch_assoc($spieler_team2b)) {
		...
		$templist .= '<a href="/jugador.php?id='.$spieler_team2c['ids'].'">'.$temp.'</a>, ';
			...
			if (count($scorers1) > 3) {
				...
				if (Chance_Percent($risikoFuerVerletzungTeam1)) {
					...
					if ($verletzter = each($verletzungenVorauswahlTeam1)) {
						...
						$torj2 = mysql_query($torj1) or reportError(mysql_error(), $torj1);
						$formulierung = '<a href="/jugador.php?id='.$verletzter['key'].'">Einer Deiner Spieler</a> fällt '.$verletzungsDaten['name'].' für '.$verletzungsDaten['dauer'].' Tage aus.';
