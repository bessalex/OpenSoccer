...
if (isset($_POST['nachricht']) && $cookie_id != CONFIG_DEMO_USER) {
	...
	if (mysql_num_rows($sql2) > 0) {
		...
		if ($chatSperreBis > 0 && $chatSperreBis > time()) {
			addInfoBox(__('Du bist noch bis zum %1$d für die Kommunikation im Spiel gesperrt. Wenn Dir unklar ist warum, frage bitte das %2$s', date('d.m.Y H:i', $chatSperreBis), '<a class="inText" href="/quien_esta_en_linea.php">'._('Support-Team.').'</a>'));
			include 'zz3.php';
			exit;
		}
	}
...
<form action="/copa.php" method="post" accept-charset="utf-8">
...
while ($sql3 = mysql_fetch_assoc($sql2)) {
	...
	if ($sql3['user'] == $cookie_id OR $_SESSION['status'] == 'Helfer' OR $_SESSION['status'] == 'Admin') {
		echo ' <a href="/copa.php?delEntry='.$sql3['id'].'">[Löschen]</a>';
	}
...
while ($wrij3 = mysql_fetch_assoc($wrij2)) {
	...
	if ($rundenarray != '') {
        if (!isset($spiele[$rundenarray][$wrij3['kennung']])) {
            ...
            $ergebnis_string = '<a href="/informe_partido.php?id='.$wrij3['id'].'">'.$ergebnis_live.'</a>';
...
for ($i = 1; $i <= 6; $i++) {
	...
	foreach ($spielliste as $spiel) {
		$tmp_liga_cache .= '<tr class="land_'.$laender[$team_ids[$spiel['team1']][1]].' land_'.$laender[$team_ids[$spiel['team2']][1]].'"><td class="link"><a href="/equipo.php?id='.$team_ids[$spiel['team1']][0].'">'.$spiel['team1'].'</a></td><td><img src="/images/flaggen/'.$laender[$team_ids[$spiel['team1']][1]].'.png" alt="" /></td><td class="link"><a href="/equipo.php?id='.$team_ids[$spiel['team2']][0].'">'.$spiel['team2'].'</a></td><td><img src="/images/flaggen/'.$laender[$team_ids[$spiel['team2']][1]].'.png" alt="" /></td><td class="link">'.$spiel['ergebnis1'].'</td><td class="link">'.$spiel['ergebnis2'].'</td></tr>';
	}
...
while ($sql3 = mysql_fetch_assoc($sql2)) {
	...
	if ($sql3['user'] == $cookie_id OR $_SESSION['status'] == 'Helfer' OR $_SESSION['status'] == 'Admin') {
		echo ' <a href="/copa.php?delEntry='.$sql3['id'].'">'._('[Löschen]').'</a>';
	}
	echo '</b><br />'.$sql3['nachricht'].'</p>';
}
?>
