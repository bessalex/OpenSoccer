...
if ($live_scoring_spieltyp_laeuft == '') {
	...
	if (count($resultTable) > 0) {
		...
		foreach ($resultTable as $spielID=>$resultEntry) {
			...
			echo '<td class="link"><a href="/informe_partido.php?id='.$spielID.'#lastAction">'._('Spielbericht').'</a></td>';
			echo '</tr>';
		}
		echo '</tbody>';
		echo '</table>';
...
<p><strong><?php echo _('Hinweis:'); ?></strong> <?php echo __('Spiele Deiner %s sind mit einem Symbol vor den Teamnamen gekennzeichnet. Deine eigenen Spiele sind fett markiert.', '<a href="/amigos.php">'._('Freunde').'</a>'); ?></p>
<?php } else { ?>
