...
echo '<form action="/cuentasMultiples.php" method="get" accept-charset="utf-8">';
...
if (mysql_num_rows($sql2) == 0) {
	...
	echo '<form action="/cuentasMultiples.php?user1='.urlencode($user1Name).'&user2='.urlencode($user2Name).'" method="post" accept-charset="utf-8">';
...
foreach ($bothIPs as $bothIP=>$userData) {
	...
	if ($bothIP == $letzteZeile) {
		...
		echo ' href="/infoIp.php?ip='.urlencode($bothIP).'">'.$bothIP.'</a></td>';
	}
	echo '<td>';
