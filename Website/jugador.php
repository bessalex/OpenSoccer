...
if ($loggedin == 1) {
    echo '<a href="/seguimiento_mercado.php?id='.$sql3['ids'].'" class="pagenava" onclick="return'.noDemoClick($cookie_id, TRUE).' confirm(\''._('Bist Du sicher?').'\')">';
    if ($watch3 == 0) {
...
echo '<a href="/historial_jugador.php?id='.$sql3['ids'].'" class="pagenava">'._('Zur Historie').'</a></p>';
?>
<table>
<thead>
<tr class="odd">
...
if ($sql3['team'] == 'frei') {
	...
	echo '<tr><td>'._('Team').'</td><td class="link"><a href="/equipo.php?id='.$sql3['team'].'">'.$tm3['name'].'</a></td></tr>';
}
...
if ($sql3['team'] == 'frei') {
	...
	elseif ($sql3['transfermarkt'] == 1) {
		echo '<a href="/subasta_mercado.php?id='.$sql3['ids'].'">'._('Zur Auktion').'</a>';
	}
	elseif ($sql3['transfermarkt'] > 999998) {
		...
		echo '<a href="/cesion_mercado.php?seite='.$getLeihPage.'&amp;mark='.$sql3['ids'].'">'._('Zur Leihgabe').'</a>';
	}
	echo '</td></tr>';
}
...
if ($loggedin == 1 && $sql3['team'] == $cookie_team && $sql3['leiher'] == 'keiner') {
	if ($sql3['marktwert'] > 0) {
		echo '<tr class="odd"><td colspan="2" class="link"><a href="/renovar_contrato.php?id='.$sql3['ids'].'">'._('Vertrag verlängern').'</a></td></tr>';
	}
	else {
	...
	if ($sql3['vertrag'] < getTimestamp('+48 hours')) {
		...
		echo '<tr><td colspan="2" class="link"><a href="/despedir_jugador.php?id='.$sql3['ids'].'" onclick="return confirm(\''._('Bist Du sicher?').'\')">'.__('Für %s € entlassen', number_format($entlassungskosten, 0, ',', '.')).'</a></td></tr>';
	}
}
?>
</tbody>
		...
		if ($_SESSION['transferGesperrt'] == FALSE) {
			...
			if ($sql3['transfermarkt'] == 0) {
				...
				if ($sql3['spiele_verein'] > 5 && $alter_in_jahren < 34) {
					echo '<form action="/accion_mercado.php" method="post" accept-charset="utf-8">';
