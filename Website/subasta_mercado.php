...
if (!isset($_GET['id'])) {
	...
	if ($sql3['besitzer'] == 'KEINER') {
		...
		$team_name = $sql9['name'];
		$teamLink = '<td class="link"><a href="/equipo.php?id='.$sql3['besitzer'].'">'.$team_name.'</a></td>';
...
<p style="text-align:right"><a href="/subasta_mercado.php?id=<?php echo $ids; ?>" onclick="window.location.reload(); return false" class="pagenava"><?php echo _('Seite aktualisieren'); ?></a></p>
<?php if ($loggedin == 1) { ?>
<?php
...
if ($_SESSION['pMaxGebot'] == 0) {
	...
	echo '<p>'.__('Du solltest Dir zuerst einmal %1$s ansehen und versuchen, eine erste %2$s daraus zu formen.', '<a href="/plantilla.php">'._('Deinen Kader').'</a>', '<a href="/alineacion.php">'._('Mannschaft').'</a>').'</p>';
	include 'zz3.php';
	exit;
}
...
if ($_SESSION['transferGesperrt'] == TRUE) {
	addInfoBox(__('Du bist noch für den Transfermarkt %1$s. Wenn Dir unklar ist, warum, frage bitte ein %2$s.', '<a class="inText" href="/sanciones.php">'._('gesperrt').'</a>', '<a class="inText" href="/escribir_correo.php?id='.CONFIG_OFFICIAL_USER.'">'._('Team-Mitglied').'</a>'));
	include 'zz3.php';
	exit;
}
...
<?php
echo '<p><table>';
echo '<thead><tr class="odd"><th scope="col" colspan="2"><a href="/jugador.php?id='.$sql3['spieler'].'">'.$spieler_name.' ('.__('%d Gebote', $sql3['gebote']).' / '.__('%d Beobachter', $cntWatcher3).')</a></th></tr></thead>';
echo '<tbody>';
