...
if ($an2a > 0) { echo '<h1>'._('Anfragen').'</h1><p><table><thead><tr class="odd"><th scope="col">'._('Manager').'</th><th scope="col">'._('Aktionen').'</th></tr></thead><tbody>';
    while ($an3 = mysql_fetch_assoc($an2)) {
        echo '<tr><td class="link">'.displayUsername($an3['username'], $an3['von']).'</td><td><form action="/accion_amigos.php" method="get" accept-charset="utf-8"><input type="hidden" name="id" value="'.$an3['von'].'" /><button type="submit" name="aktion" value="Annehmen"'.noDemoClick($cookie_id).'>'._('Annehmen').'</button> <button type="submit" name="aktion" value="Ablehnen"'.noDemoClick($cookie_id).'>'._('Ablehnen').'</button></form></td>';
    }
    echo '</tbody></table></p>';
}
?>
...
if ($kontakt2a == 'noch keine') { echo '<p>'._('Wenn Du Deine Freunde zu dieser Liste hinzufügen möchtest, dann öffne bitte das Profil des jeweiligen Managers und klicke unten auf den Link &quot;Freundschaft anbieten&quot;.').'</p>'; }
    ...
    while ($kontakt3 = mysql_fetch_assoc($kontakt2)) {
        ...
        echo '<tr><td class="link">'.displayUsername($kontakt3['username'], $kontakt3['f2']).'</td><td>'.date('d.m.Y, H:i', $kontakt3['last_login']).' Uhr</td><td class="link"><a href="/escribir_correo.php?id='.$kontakt3['f2'].'">Nachricht schicken</a></td>';
		echo '<td><a href="/amigos.php?sort='.$sortUp.'&amp;user='.$kontakt3['f2'].'"><img src="/images/arrow_up.png" alt="+" title="'._('Nach oben verschieben').'" /></a> <a href="/amigos.php?sort='.$sortDown.'&amp;user='.$kontakt3['f2'].'"><img src="/images/arrow_down.png" alt="-" title="'._('Nach unten verschieben').'" /></a></td>';
		echo '</tr>';
    }
    echo '</tbody></table></p>';
}
