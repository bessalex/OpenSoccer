...
if (GameTime::getMatchDay() > 11) { // Hinrunden-Tabelle der aktuellen Saison
	$linkText = _('Hinrunde');
	$linkURL = '/est_historia.php?saison_spieltag='.GameTime::getSeason().'-11&liga='.$temp_liga;
}
else { // End-Tabelle der letzten Saison
	$linkText = _('Letzte Saison');
	$linkURL = '/est_historia.php?saison_spieltag='.intval(GameTime::getSeason()-1).'-22&liga='.$temp_liga;
}
...
$tmp_liga_cache .= '
<h1>'._('Tabelle').'</h1>
<p style="text-align:right"><a href="/est_goleadores.php?liga='.$temp_liga.'" class="pagenava">'._('Torjäger').'</a> <a href="'.$linkURL.'" class="pagenava">'.$linkText.'</a></p>
<table>
<thead>
<tr class="odd">
...
while ($sql3 = mysql_fetch_assoc($sql2)) {
	...
	$tmp_liga_cache .= '</td><td class="link"><a href="/equipo.php?id='.$sql3['ids'].'">'.$sql3['name'].' ('.number_format($sql3['aufstellung'], 1, ',', '.').')';
...
<form action="/lig_tabla.php" method="get" accept-charset="utf-8">
...
while ($erg3 = mysql_fetch_assoc($erg2)) {
	...
	echo '<td>'.date('d.m.Y', $erg3['datum']).'</td><td>'.$erg3['team1'].'</td><td>'.$erg3['team2'].'</td><td class="link"><a href="/informe_partido.php?id='.$erg3['id'].'">'.$ergebnis_live.'</a></td>';
	echo '</tr>';
	$counter++;
}
echo '</tbody>';
...
<form action="/lig_tabla.php" method="post" accept-charset="utf-8">
...
while ($sql3 = mysql_fetch_assoc($sql2)) {
	...
	if ($loggedin == 1 && ($sql3['user'] == $cookie_id || $_SESSION['status'] == 'Helfer' || $_SESSION['status'] == 'Admin')) {
		echo ' <a href="/lig_tabla.php?liga='.mysql_real_escape_string($temp_liga).'&amp;delEntry='.$sql3['id'].'">'._('[Löschen]').'</a>';
	}
