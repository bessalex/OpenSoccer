...
if ($_SESSION['status'] == 'Helfer' OR $_SESSION['status'] == 'Admin') {
	...
	if (isset($_GET['user']) && isset($_GET['reporter']) && isset($_GET['datum'])) {
		...
		while ($sql3 = mysql_fetch_assoc($sql2)) {
			...
			echo '<td class="link"><a href="/reportes_chat.php?user='.urlencode($sql3['user']).'&amp;reporter='.urlencode($sql3['reporter']).'&amp;datum='.urlencode($sql3['datum']).'">'.$sql3['datum'].'</a></td>';
			if ($sql3['sperrRelevant'] == 1) {
				echo '<td class="link"><a href="/reportes_chat.php?seite='.intval($seite).'&amp;aufheben='.urlencode($sql3['user']).'&amp;reporter='.urlencode($sql3['reporter']).'&amp;datum='.urlencode($sql3['datum']).'" onclick="return confirm(\''._('Bist Du sicher?').'\')">'._('Aufheben').'</a></td>';
			}
			else {
				echo '<td>&nbsp;</td>';
			}
