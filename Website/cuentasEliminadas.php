...
if ($_SESSION['status'] == 'Helfer' OR $_SESSION['status'] == 'Admin') {
	...
	while ($sql3 = mysql_fetch_assoc($sql2)) {
		...
		echo '<td class="link"><a href="/infoIp.php?ip='.$sql3['ip'].'">'.$sql3['username'].'</a></td><td>'.date('d.m.Y H:i', $sql3['zeit']).'</td><td>'.round($sql3['dabei']/86400).' Tage</td><td class="link"><a href="/lig_tabla.php?liga='.$sql3['liga'].'">'.$sql3['name'].'</a></td>';
		echo '</tr>';
		$counter++;
	}
	echo '</tbody></table>';
