...
if ($_SESSION['transferGesperrt'] == TRUE) {
    addInfoBox(__('Du bist noch für den Transfermarkt %1$s. Wenn Dir unklar ist, warum, frage bitte ein %2$s.', '<a class="inText" href="/sanciones.php">'._('gesperrt').'</a>', '<a class="inText" href="/escribir_correo.php?id=18a393b5e23e2b9b4da106b06d8235f3">'._('Team-Mitglied').'</a>'));
}
else {
			...
			if (mysql_num_rows($bid2) != 0) {
				...
				if (mysql_affected_rows() != 0) {
					...
					if ($transfers_mit_team[$ac_bieter_id] < 2) {
						...
						$formulierung = __('Du hast den Spieler %s an einen anderen Verein verliehen.', '<a href="/jugador.php?id='.$ac_spieler.'">'.$getmanager4.'</a>'.$ac_praemie_str);
						$sql7 = "INSERT INTO ".$prefix."protokoll (team, text, typ, zeit) VALUES ('".$cookie_team."', '".$formulierung."', 'Transfers', ".time().")";
						$sql8 = mysql_query($sql7);
						$formulierung = __('Du hast den Spieler %s ausgeliehen.', '<a href="/jugador.php?id='.$ac_spieler.'">'.$getmanager4.'</a>'.$ac_praemie_str);
...
if ($an2a > 0) { echo '<p><table><thead><tr class="odd"><th scope="col">'._('Spieler').'</th><th scope="col">'._('Team').'</th><th scope="col">'._('Prämie').'</th><th scope="col">'._('Aktion').'</th></tr></thead><tbody>';
    while ($an3 = mysql_fetch_assoc($an2)) {
		...
		echo '<tr><td class="link"><a href="/jugador.php?id='.$an3['spieler'].'">'.$an3['vorname'].' '.$an3['nachname'].'</a></td><td class="link"><a href="/equipo.php?id='.$an3['besitzer'].'">'.$tlink3['name'].'</a></td><td>'.number_format($an3['praemie'], 0, ',', '.').' €</td>';
		echo '<td><form action="/cesiones.php" method="POST" accept-charset="utf-8"><input type="hidden" name="spieler" value="'.$an3['spieler'].'" /><input type="hidden" name="besitzer" value="'.$an3['besitzer'].'" /><input type="submit" name="aktion" value="'._('Zurückziehen').'"'.noDemoClick($cookie_id).' /></form></td>';
    }
    echo '</tbody></table></p>';
}
else {
...
while ($sql3 = mysql_fetch_assoc($sql2)) {
	...
	echo '</td><td>'.$sql3['position'].'</td><td class="link"><a href="/jugador.php?id='.$sql3['ids'].'">'.$sql3['vorname'].' '.$sql3['nachname'].'</a></td><td>'.floor($sql3['wiealt']/365).'</td><td>'.number_format($sql3['staerke'], 1, ',', '.').'</td><td>'.$sql3['spiele'].'</td>';
	echo '<td class="link"><a href="/equipo.php?id='.$sql3['team'].'">'.$sql3['name'].'</a></td>';
...
while ($sql3 = mysql_fetch_assoc($sql2)) {
	if ($counter % 2 == 0) { echo '<tr>'; } else { echo '<tr class="odd">'; }
	echo '</td><td>'.$sql3['position'].'</td><td class="link"><a href="/jugador.php?id='.$sql3['ids'].'">'.$sql3['vorname'].' '.$sql3['nachname'].'</a></td><td>'.floor($sql3['wiealt']/365).'</td><td>'.number_format($sql3['staerke'], 1, ',', '.').'</td><td>'.$sql3['spiele'].'</td>';
	echo '<td class="link"><a href="/equipo.php?id='.$sql3['leiher'].'">'.$sql3['name'].'</a></td>';
	echo '<td>'.number_format($sql3['praemieProEinsatz'], 0, ',', '.').' €</td>';
	echo '</tr>';
	$counter++;
}
