...
if (mysql_num_rows($sql2) == 0) {
	...
	while ($sql3 = mysql_fetch_assoc($sql2)) {
		...
		echo '<td class="link"><a href="/jugador.php?id='.$sql3['ids'].'">'.$sql3['vorname'].' '.$sql3['nachname'].'</a></td>';
		echo '<td>'.$sql3['gebote'].'</td>';
		echo '<td class="link"><a href="/subasta_mercado.php?id='.$sql3['ids'].'">'.number_format($sql3['betrag_highest'], 0, ',', '.').' €</a></td>';
