...
<h1><?php echo _('Team:').' '.$sql3['name']; ?></h1>
	...
	if ($loggedin == 1 && ($_SESSION['status'] == 'Helfer' || $_SESSION['status'] == 'Admin')) {
		...
		echo '<a href="/cambioNombre.php?team='.$clearid.'" class="pagenava">'._('Vereinsnamen ändern').'</a>';
		echo '<a href="/protocolo.php?team='.$clearid.'" class="pagenava">'._('Protokoll ansehen').'</a>';
		echo '</p>';
	}
	?>
<p style="text-align:right">
	<a href="/lig_transferencias.php?team=<?php echo $clearid; ?>" class="pagenava"><?php echo _('Transfers dieses Teams'); ?></a>
	<a href="/calendario.php?team=<?php echo $clearid; ?>" class="pagenava"><?php echo _('Zum Spielplan'); ?></a>
</p>
<table>
<thead>
<tr class="odd">
...
echo '<tr><td>'._('Land').'</td><td class="link"><a href="/lig_tabla.php?liga='.$sql3['liga'].'">'.$tql3['name'].'</a></td></tr>';
...
while ($sql6 = mysql_fetch_assoc($sql5)) {
	...
	echo '</td>';
	echo '<td class="link"><a href="/jugador.php?id='.$sql6['ids'].'">'.$sql6['vorname'].' '.$sql6['nachname'].'</a></td><td>'.floor($sql6['wiealt']/365).'</td><td>'.number_format($sql6['staerke'], 1, ',', '.').' <span style="color:#999">('.number_format($schaetzungVomScout, 1, ',', '.').')</span></td><td>'.$sql6['frische'].'%</td>';
	echo '</tr>';
	$counter++;
}
