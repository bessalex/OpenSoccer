...
if (isset($_POST['laufzeit']) && isset($_POST['spieler'])) {
	...
	if ($laufzeit == 22 || $laufzeit == 44 || $laufzeit == 66) {
		if ($cookie_id != CONFIG_DEMO_USER) {
            ...
            if (isContractOptionAvailable($inc, $laufzeit)) {
                ...
                $formulierung = __('Du hast den Vertrag mit %1$s auf %2$d Tage verlängert.', '<a href="/jugador.php?id='.$spieler_id.'">'.$getmanager4.'</a>', $laufzeit);
    ...
    if (isset($_POST['returnToVertraege'])) {
        ...
        header ('Location: /jugador.php?id='.$spieler_id);
    }
    exit;
}
?>
...
<?php if ($loggedin == 1) { ?>
<form action="/renovar_contrato.php" method="post" accept-charset="utf-8" class="imtext">
<?php
...
<p>
<?php if (isset($_SERVER['HTTP_REFERER'])) { if (stripos($_SERVER['HTTP_REFERER'], '/contratos.php') !== false) { ?><input type="hidden" name="returnToVertraege" value="1" /><?php } } ?>
...
<p><?php echo __('Der Spieler %s möchte seinen Vertrag zurzeit nicht verlängern und hat Dir deshalb kein Angebot gemacht. Vielleicht ist er unzufrieden in Deinem Team. Achte auf seine Moral!', '<a href="/jugador.php?id='.$sql3['ids'].'">'.$sql3['vorname'].' '.$sql3['nachname'].'</a>'); ?></p>
<p><a href="/contratos.php"><?php echo _('Zurück zur Vertragsübersicht'); ?></a><br /><?php echo '<a href="/jugador.php?id='.$sql3['ids'].'">'._('Zurück zum Spielerprofil').'</a>'; ?></p>
<?php } ?>
<?php } else { ?>
