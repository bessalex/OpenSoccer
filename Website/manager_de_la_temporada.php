...
?>
<form action="/manager_de_la_temporada.php" method="get" accept-charset="utf-8">
...
while ($sql3 = mysql_fetch_assoc($sql2)) {
	...
	$link_zur_tabelle = '/est_historia.php?saison_spieltag='.intval(GameTime::getSeason()-1).'-22&liga='.$sql3['vorjahr_liga'];
	$bilanz = round($sql3['gewinnGeld']/1000000);
	if ($bilanz > 0) { $bilanz = '+'.$bilanz; }
	echo '<td>'.displayUsername($sql3['username'], $sql3['ids']).' (<a href="/equipo.php?id='.$sql3['team'].'">'.$sql3['name'].'</a>)</td>';
	...
	echo '<td class="link"><a href="/manager_de_la_temporada.php?wahl_id='.$sql3['ids'].'&amp;sec_id='.$sec_id.'" onclick="return confirm('._('\'Bist Du sicher?\'').')">'._('Klicken').'</a></td>';
	echo '</tr>';
	$counter++;
}
?>
