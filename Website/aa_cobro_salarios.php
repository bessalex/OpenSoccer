...
while ($frei3 = mysql_fetch_assoc($frei2)) {
	...
	if (($frei3['wiealt']/365) > 34) {
		...
		$formulierung = 'Der Spieler <a href="/jugador.php?id='.$frei3['ids'].'">'.$getmanager4.'</a> hat seine aktive Zeit beendet und arbeitet nun als '.$jobsNachKarriere[0].' in Deinem Verein.';
	}
	else {
		$formulierung = 'Der Vertrag des Spielers <a href="/jugador.php?id='.$frei3['ids'].'">'.$getmanager4.'</a> ist ausgelaufen. Er spielt jetzt außerhalb Europas.';
	}
