...
<?php if ($loggedin == 1) { ?>
<p style="text-align:right"><a href="/deseosCambioLiga.php" class="pagenava"><?php echo _('Tausch-Wünsche'); ?></a></p>
<?php
...
elseif ($live_scoring_spieltyp_laeuft != '') {
	...
	if (mysql_num_rows($sql2) == 0) {
		...
		while ($sql3 = mysql_fetch_assoc($sql2)) {
			echo '<tr class="odd">';
			echo '<td class="link"><a href="/equipo.php?id='.$sql3['vonTeam'].'">'.$sql3['name'].'</a></td>';
			echo '<td>'.date('d.m.Y H:i', $sql3['zeit']).'</td>';
			echo '<td><form action="/cambioLiga.php" method="post" accept-charset="utf-8"><input type="hidden" name="newTeam" value="'.$sql3['vonTeam'].'" /><button type="submit" name="aktion" value="Annehmen"'.noDemoClick($cookie_id).'>'._('Annehmen').'</button> <button type="submit" name="aktion" value="Ablehnen"'.noDemoClick($cookie_id).'>'._('Ablehnen').'</button></form></td>';
			echo '</tr>';
			$counter++;
		}
		echo '</tbody></table></p>';
	...
	<form action="/cambioLiga.php" method="post" accept-charset="utf-8">
	...
	if (mysql_num_rows($sql2) == 0) {
		...
		while ($sql3 = mysql_fetch_assoc($sql2)) {
			echo '<tr class="odd">';
			echo '<td class="link"><a href="/equipo.php?id='.$sql3['anTeam'].'">'.$sql3['name'].'</a></td>';
			echo '<td>'.date('d.m.Y H:i', $sql3['zeit']).'</td>';
