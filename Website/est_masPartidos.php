...
$tmp_liga_cache = '';
    ...
    while ($torj3 = mysql_fetch_assoc($torj2)) {
        ...
        $tmp_liga_cache .= '<td>'.$counter.'</td><td class="link"><a href="/jugador.php?id='.$torj3['ids'].'">'.$torj3['vorname'].' '.$torj3['nachname'].'</a></td><td class="link"><a href="/equipo.php?id='.$torj3['team'].'">'.$torj3['name'].'</a></td><td>'.$torj3['spiele'].'</td>';
        $tmp_liga_cache .= '</tr>';
        $counter++;
    }
