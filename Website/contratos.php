...
while ($sql3 = mysql_fetch_assoc($sql2)) {
	...
	echo '<td'.$farbcode.'>&nbsp;</td>';
	echo '<td class="link"><a href="/jugador.php?id='.$sql3['ids'].'">'.mb_substr($sql3['vorname'], 0, 1, 'utf-8').'. '.$sql3['nachname'].'</a></td>';
	...
	if ($sql3['marktwert'] > 0 && $sql3['leiher'] == 'keiner') {
		echo '<td class="link"><a href="/renovar_contrato.php?id='.$sql3['ids'].'">'._('Verlängern').'</a></td>';
	}
	else {
		echo '<td>&nbsp;</td>';
	}
