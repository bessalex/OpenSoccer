...
$pw_meldung = _('Dein Passwort konnte leider nicht geändert werden. Bitte versuche es noch einmal!');
    ...
    if ($pw_neu1 == $pw_neu2) {
        if (mb_strlen($pw_neu1) >= 6) {
            if ($pw_neu1 != $pw_alt) {
                $getPw1 = "SELECT password FROM ".$prefix."users WHERE ids = '".$cookie_id."'";
                $getPw2 = mysql_query($getPw1);
                if ($getPw2 && mysql_num_rows($getPw2) > 0) {
                    $getPw3 = mysql_fetch_assoc($getPw2);
                    $db_pw = $getPw3['password'];
                    $old_valid = false;
                    if (password_verify($pw_alt, $db_pw) || $db_pw === md5('1'.$pw_alt.'29')) {
                        $old_valid = true;
                    }

                    if ($old_valid) {
                        $pw_neu_hash = password_hash($pw_neu1, PASSWORD_DEFAULT);
                        $sql1 = "UPDATE ".$prefix."users SET password = '".mysql_real_escape_string($pw_neu_hash)."' WHERE ids = '".$cookie_id."'";
                        $sql2 = mysql_query($sql1);
                        if ($sql2 != FALSE) {
