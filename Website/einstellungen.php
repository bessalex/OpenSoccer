...
$pw_meldung = _('Dein Passwort konnte leider nicht geändert werden. Bitte versuche es noch einmal!');
    ...
    if ($pw_neu1 == $pw_neu2) {
        if (mb_strlen($pw_neu1) >= 6) {
            if ($pw_neu1 != $pw_alt) {
                $getUserPw1 = "SELECT password FROM ".$prefix."users WHERE ids = '".$cookie_id."'";
                $getUserPw2 = mysql_query($getUserPw1);
                if ($getUserPw2 && mysql_num_rows($getUserPw2) == 1) {
                    $getUserPw3 = mysql_fetch_assoc($getUserPw2);
                    $storedPw = $getUserPw3['password'];
                    $pwValid = FALSE;
                    if (password_verify($pw_alt, $storedPw)) {
                        $pwValid = TRUE;
                    }
                    else {
                        $legacyHash = md5('1'.$pw_alt.'29');
                        if ($storedPw === $legacyHash) {
                            $pwValid = TRUE;
                        }
                    }
                    if ($pwValid) {
                        $pw_neu = mysql_real_escape_string(password_hash($pw_neu1, PASSWORD_DEFAULT));
