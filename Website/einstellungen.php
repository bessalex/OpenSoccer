...
$pw_meldung = _('Dein Passwort konnte leider nicht geändert werden. Bitte versuche es noch einmal!');
    ...
    if ($pw_neu1 == $pw_neu2) {
        if (mb_strlen($pw_neu1) >= 6) {
            if ($pw_neu1 != $pw_alt) {
                $checkPwSql1 = "SELECT password FROM ".$prefix."users WHERE ids = '".$cookie_id."'";
                $checkPwSql2 = mysql_query($checkPwSql1);
                if ($checkPwSql2 && mysql_num_rows($checkPwSql2) > 0) {
                    $userPwRow = mysql_fetch_assoc($checkPwSql2);
                    $currentHashInDb = $userPwRow['password'];
                    $pw_valid = password_verify($pw_alt, $currentHashInDb) || ($currentHashInDb === md5('1'.$pw_alt.'29'));
                    if ($pw_valid) {
                        $pw_neu_hash = mysql_real_escape_string(password_hash($pw_neu1, PASSWORD_DEFAULT));
                        $sql1 = "UPDATE ".$prefix."users SET password = '".$pw_neu_hash."' WHERE ids = '".$cookie_id."'";
                        $sql2 = mysql_query($sql1);
                        if ($sql2 != FALSE) {
