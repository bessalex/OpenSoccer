...
while ($kontakt3 = mysql_fetch_assoc($kontakt2)) {
	...
	echo '<td class="link">'.$mark1.'<a href="/equipo.php?id='.$kontakt3['team'].'">'.$kontakt3['name'].'</a>'.$mark2.'</td>';
	echo '<td class="link">'.$mark1.'<a href="/escribir_correo.php?id='.$kontakt3['ids'].'">'._('Post schicken').'</a>'.$mark2.'</td></tr>';
}
?>
</tbody></table>
<p><strong><?php echo _('Hinweis:'); ?></strong> <?php echo __('Deine %s sind fett markiert.', '<a href="/amigos.php">'._('Freunde').'</a>'); ?></p>
...
while ($kontakt3 = mysql_fetch_assoc($kontakt2)) {
	...
	echo '<td class="link"><a href="/escribir_correo.php?id='.$kontakt3['ids'].'">'._('Post schicken').'</a></td></tr>';
}
?>
</tbody></table>
