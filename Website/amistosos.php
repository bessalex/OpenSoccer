...
echo '<form action="/amistosos.php" method="post" accept-charset="utf-8">';
	...
	if ($an2a > 0) {
		...
		while ($an3 = mysql_fetch_assoc($an2)) {
			if (in_array($an3['datum'], $testspiel_tage) && $an3['datum'] > time()) { // wenn an dem Tag schon ein Testspiel ist
				...
				$formulierung = __('Der Co-Trainer von %s hat Dein Angebot für ein Testspiel abgelehnt.', '<a href="/equipo.php?id='.$cookie_team.'">'.$cookie_teamname.'</a>');
				...
				echo '<tr><td class="link"><a href="/equipo.php?id='.$an3['team1'].'">'.$an3['team1_name'].'</a></td><td>'.date('d.m.Y', $an3['datum']).'</td>';
				echo '<td><form action="/respuestas_amistoso.php" method="get" accept-charset="utf-8"><input type="hidden" name="id" value="'.$an3['team1'].'" /> <button type="submit" name="typ" value="Annehmen"'.noDemoClick($cookie_id).'>'._('Annehmen').'</button> <button type="submit" name="typ" value="Ablehnen"'.noDemoClick($cookie_id).'>'._('Ablehnen').'</button></form></td>';
				echo '</tr>';
			}
		}
		echo '</tbody></table></p>';
...
if ($an2a > 0) {
    ...
    while ($an3 = mysql_fetch_assoc($an2)) {
        echo '<tr><td class="link"><a href="/equipo.php?id='.$an3['team2'].'">'.$an3['name'].'</a></td><td>'.date('d.m.Y', $an3['datum']).'</td><td class="link"><a href="/amistosos.php?recall='.$an3['team2'].'" onclick="return confirm(\'Bist Du sicher?\')">Zurückziehen</a></td>';
    }
    echo '</tbody></table></p>';
}
else {
...
if ($an2a > 0) { echo '<p><table><thead><tr class="odd"><th scope="col">'._('Gegner').'</th><th scope="col">'._('Datum').'</th><th scope="col">'._('Ergebnis').'</th></tr></thead><tbody>';
    while ($an3 = mysql_fetch_assoc($an2)) {
		...
		echo '<td class="link"><a href="/informe_partido.php?id='.$an3['id'].'">'.$ergebnis_live.'</a></td>';
		echo '</tr>';
    }
    echo '</tbody></table></p>';
}
