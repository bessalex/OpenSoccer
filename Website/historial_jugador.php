...
if (mysql_num_rows($tm2) == 0) {
	...
	$aktuellerVerein = '<a href="/equipo.php?id='.$sql3['team'].'">'.$tm3['name'].'</a>';
}
?>
...
<p style="text-align:right"><a href="/jugador.php?id=<?php echo $sql3['ids']; ?>" class="pagenava"><?php echo _('Zum Spieler-Profil'); ?></a></p>
<table>
<thead>
<tr class="odd">
...
if (mysql_num_rows($youthTeam2) == 1) {
	...
	if ($counter % 2 == 0) { echo '<tr>'; } else { echo '<tr class="odd">'; }
	echo '<td>'._('Jugend').'</td><td colspan="3"><a href="/equipo.php?id='.$sql3['jugendTeam'].'">'.$youthTeam3['name'].'</a></td>';
	echo '</tr>';
}
// JUGEND-TEAM ENDE
?>
...
while ($a3 = mysql_fetch_assoc($a2)) {
	...
	if ((time()-$a3['datum']) > 259200) {
		echo '<a href="/equipo.php?id='.$a3['bieter'].'">'.date('d.m.Y H:i:s', $a3['datum']).'</a>';
	}
	else {
		echo date('d.m.Y H:i:s', $a3['datum']);
	}
	echo '</td>';
	echo '<td class="link"><a href="/infoIp.php?ip='.urlencode($a3['bieterIP']).'">'.$a3['bieterIP'].'</a></td>';
	if ($a3['betrag'] == 1) {
		echo '<td>'._('Leihe').'</td>';
	}
	else {
