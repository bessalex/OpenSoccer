...
<?php include 'zz2.php'; ?>
<?php include 'zzsubnav_estadisticas.php'; ?>
<h1><?php echo _('Treueste Spieler'); ?></h1>
<?php if ($loggedin == 1) { ?>
<?php
// NUR EIGENE SPIELER ODER ALLE ANFANG
$onlyShowOwnSQL = "";
$onlyShowOwnLink = '<a href="/est_jugadoresMasFieles.php?own=1" class="pagenava">'._('Meine Spieler').'</a>';
$onlyShowOwnCache = '';
if (isset($_GET['own'])) {
	if ($_GET['own'] == '1') {
		$onlyShowOwnSQL = " WHERE a.team = '".$cookie_team."'";
		$onlyShowOwnLink = '<a href="/est_jugadoresMasFieles.php?own=0" class="pagenava">'._('Alle Spieler').'</a>';
...
$tmp_liga_cache = '';
    ...
    while ($torj3 = mysql_fetch_assoc($torj2)) {
        ...
        $tmp_liga_cache .= '<td>'.$counter.'</td><td class="link"><a href="/jugador.php?id='.$torj3['ids'].'">'.$torj3['vorname'].' '.$torj3['nachname'].'</a></td><td class="link"><a href="/equipo.php?id='.$torj3['team'].'">'.$torj3['name'].'</a></td><td>'.$torj3['spiele_verein'].'</td>';
        $tmp_liga_cache .= '</tr>';
        $counter++;
    }
