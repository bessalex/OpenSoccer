...
if (mysql_num_rows($get_urlaub5) > 0) {
	...
	if ($get_urlaub6['ende'] > time()) {
        ...
        $aktueller_urlaub .= '<form action="/configuracion.php" method="post" accept-charset="utf-8"><input type="hidden" name="urlaub_abbrechen" value="1" /><input type="submit" value="'._('Urlaub abbrechen').'" onclick="return'.noDemoClick($cookie_id, TRUE).' confirm(\''._('Bist Du sicher?').'\')" /></form>';
	}
	else {
		$aktueller_urlaub = '';
	}
...
<p><?php echo __('Du möchtest den Namen Deines Vereins ändern? Dann kannst Du das %s tun!', '<a href="/cambioNombre.php">'._('auf dieser Seite').'</a>'); ?></p>
<p><?php echo __('Eine Liste mit %s findest Du hier. Diese Namen darfst Du leider nicht für Dein Team verwenden.', '<a href="/nombresEquipoBloqueados.php">'._('geschützten Namen').'</a>'); ?></p>
<?php } ?>

<h1><?php echo _('Managernamen ändern'); ?></h1>
<p><?php echo __('Du möchtest Deinen eigenen Managernamen ändern? Schreibe bitte einfach einem %1$s eine kurze Nachricht. Dein Name wird dann so bald wie möglich geändert, wenn er nicht gegen die %2$s verstößt.', '<a href="/quien_esta_en_linea.php#teamList">'._('Mitglied des Support-Teams').'</a>', '<a href="/reglas.php">'._('Regeln').'</a>').'</p><p><strong>'._('Achtung:').'</strong> '._('Beim nächsten Login solltest Du Dich mit Deiner E-Mail-Adresse einloggen oder darauf achten, auch den neuen Namen zu versuchen. Das Einloggen mit dem alten Managernamen ist dann nicht mehr möglich.'); ?></p>
