...
if (isset($_POST['spieler'])) {
	...
	$hadresse = 'Location: /jugador.php?id='.$spieler_id;
}
else {
	exit;
}
        ...
        if ($sql2a > 0) {
            ...
            if ($sperre3 > 13) {
                if ($_POST['typ'] == 'Kauf') { // Verkaufen
					...
					$buch2 = mysql_query($buch1);
					$formulierung = __('Du hast den Spieler %1$s für %2$s € an einen Verein außerhalb Europas verkauft.', '<a href="/jugador.php?id='.$spieler_id.'">'.$spielerName.'</a>', number_format($sql3['marktwert'], 0, ',', '.'));
