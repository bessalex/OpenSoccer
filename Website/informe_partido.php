...
if ($live_scoring_meldung != '') {
	...
	echo '<a href="/informe_partido.php?id='.$sql3['id'].'" onclick="location.reload(); return false" class="pagenava">'._('Aktualisieren').'</a>';
	echo '<a href="/centroEnVivo.php" class="pagenava">'._('Zur LIVE-Zentrale').'</a>';
	echo '</p>';
}
// TORSCHUTZEN ANFANG
if ($sql3['tore1'] == '') {
    ...
    foreach ($tore1 as $tor1) {
        ...
        $torschuetzen1 = $torschuetzen1.'<a href="/jugador.php?id='.$tor1.'">'.$temp3['vorname'].' '.$temp3['nachname'].'</a>, ';
    }
...
if ($sql3['tore2'] == '') {
    ...
    foreach ($tore2 as $tor2) {
        ...
        $temp2 = mysql_query($temp1);
        $temp3 = mysql_fetch_assoc($temp2);
        $torschuetzen2 = $torschuetzen2.'<a href="/jugador.php?id='.$tor2.'">'.$temp3['vorname'].' '.$temp3['nachname'].'</a>, ';
    }
...
if ($sql3['karte_gelb1'] == '') {
    ...
    foreach ($karte_gelb1 as $karte_gelb_einzeln2) {
        ...
        $temp2 = mysql_query($temp1);
        $temp3 = mysql_fetch_assoc($temp2);
        $gelb1 = $gelb1.'<a href="/jugador.php?id='.$karte_gelb_einzeln2.'">'.$temp3['vorname'].' '.$temp3['nachname'].'</a>, ';
    }
...
if ($sql3['karte_gelb2'] == '') {
    ...
    foreach ($karte_gelb2 as $karte_gelb_einzeln2) {
        ...
        $temp2 = mysql_query($temp1);
        $temp3 = mysql_fetch_assoc($temp2);
        $gelb2 = $gelb2.'<a href="/jugador.php?id='.$karte_gelb_einzeln2.'">'.$temp3['vorname'].' '.$temp3['nachname'].'</a>, ';
    }
...
if ($sql3['karte_rot1'] == '') {
    ...
    foreach ($karte_rot1 as $karte_rot_einzeln2) {
        ...
        $temp2 = mysql_query($temp1);
        $temp3 = mysql_fetch_assoc($temp2);
        $rot1 = $rot1.'<a href="/jugador.php?id='.$karte_rot_einzeln2.'">'.$temp3['vorname'].' '.$temp3['nachname'].'</a>, ';
    }
...
if ($sql3['karte_rot2'] == '') {
    ...
    foreach ($karte_rot2 as $karte_rot_einzeln2) {
        ...
        $temp2 = mysql_query($temp1);
        $temp3 = mysql_fetch_assoc($temp2);
        $rot2 = $rot2.'<a href="/jugador.php?id='.$karte_rot_einzeln2.'">'.$temp3['vorname'].' '.$temp3['nachname'].'</a>, ';
    }
...
if ($sql3['typ'] == 'Liga') {
	...
	if (mysql_num_rows($hinrueck2) == 1) {
		...
		if ($live_scoring_spieltyp_laeuft == 'Liga' && date('d.m.Y', time()) == date('d.m.Y', $hinrueck3['datum'])) {
			...
			$hinrueckStr = ' (<a href="/informe_partido.php?id='.$hinrueck3['id'].'">'.$hinRueckCase.': '.ergebnis_drehen($hinrueck3['ergebnis']).'</a>)';
		}
	}
}
...
<tbody>
<tr><td><?php echo _('Begegnung'); ?></td><td><?php echo '<a href="/equipo.php?id='.$tid1.'">'.$sql3['team1'].'</a>'; ?> - <?php echo '<a href="/equipo.php?id='.$tid2.'">'.$sql3['team2'].'</a>'; ?></td></tr>
