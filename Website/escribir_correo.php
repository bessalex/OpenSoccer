...
elseif (isset($_GET['id'])) {
	...
	if ($loggedin == 1 AND $an != $cookie_id) {
		...
		if ($em3['status'] == 'Helfer' || $em3['status'] == 'Admin') { // Gesperrte User duerfen dem Team trotzdem schreiben
			...
			if (mysql_num_rows($sql2) > 0) {
				...
				if ($chatSperreBis > 0 && $chatSperreBis > time()) {
					addInfoBox('Du bist noch bis zum '.date('d.m.Y H:i', $chatSperreBis).' Uhr für die Kommunikation im Spiel gesperrt. Wenn Dir unklar ist warum, frage bitte das <a class="inText" href="/quien_esta_en_linea.php">Support-Team.</a>');
					include 'zz3.php';
					exit;
				}
			}
		...
		if ($an == CONFIG_OFFICIAL_USER) {
			...
			echo '<form method="post" action="/escribir_correo.php" accept-charset="utf-8">';
