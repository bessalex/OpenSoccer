...
if ($_SESSION['status'] == 'Helfer' || $_SESSION['status'] == 'Admin') {
	...
	echo '<p>Wenn ein Manager sich eine Namensänderung wünscht oder der aktuelle Name gegen die <a href="/reglas.php">Regeln</a> verstößt, kannst Du den Namen des Managers hier ändern.</p>';
	echo '<form action="/sanciones.php" method="post" accept-charset="utf-8">';
...
while ($sql3 = mysql_fetch_assoc($sql2)) {
	...
	if ($_SESSION['status'] == 'Helfer' OR $_SESSION['status'] == 'Admin' && ($sql3['transferSperre'] != $sql3['zeit'] && $sql3['chatSperre'] != $sql3['zeit'] && $sql3['geldStrafe'] != -1)) { // nicht aufgehoben
		echo '<td class="link"><a title="Sperre aufheben" href="/sanciones.php?un1='.$sql3['managerBestrafen'].'&amp;un2='.$sql3['zeit'].'" onclick="return confirm(\'Soll diese Sperre wirklich aufgehoben werden?\')"><img src="/images/fehler.png" alt="" /></a></td>';
	}
	else {
		echo '<td>&nbsp;</td>';
	}
...
if ($_SESSION['status'] == 'Helfer' || $_SESSION['status'] == 'Admin') {
	...
	echo '<form action="/sanciones.php" method="post" accept-charset="utf-8" id="form_email_all_users" style="display:none;">';
	...
	if (mysql_num_rows($sql2) > 0) {
		while ($sql3 = mysql_fetch_assoc($sql2)) {
			echo '<li>'.displayUsername($sql3['username'], $sql3['helferID']).' hat '.number_format($sql3['betrag'], 0, ',', '.').'€ an <a href="/equipo.php?id='.$sql3['teamID'].'">'.$sql3['name'].'</a> gezahlt.<ul>';
    ...
    if (mysql_num_rows($invalidMatches2) > 0) {
        ...
        while ($invalidMatches3 = mysql_fetch_assoc($invalidMatches2)) {
            echo '<li><a href="/informe_partido.php?id='.$invalidMatches3['id'].'">'.$invalidMatches3['team1'].' - '.$invalidMatches3['team2'].'</a><ul>';
