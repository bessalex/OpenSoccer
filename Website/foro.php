...
<form action="/foro.php" method="get" accept-charset="utf-8">
...
verbessern. Danke!</p>
<p style="text-align:right"><a href="/foro.php?mark=read" class="pagenava" onclick="return confirm('Bist Du sicher?')">Alle als gelesen markieren</a> <a href="#" class="pagenava">Neues Thema</a><!-- /forum_eintrag_hinzufuegen.php --></p>
<table>
<thead>
<tr class="odd">
...
while ($sql3 = mysql_fetch_assoc($sql2)) {
	...
	$pageLink = '/foro_tema.php?id='.$sql3['ids'].'&amp;seite='.$letzte_seite;
	// IMMER LINK AUF LETZTE SEITE ENDE
	echo '<tr'.$faerbung.'><td>'.$sticky.$mark1.'<a'.$sichtbar_fuer_zusatz.' href="'.$pageLink.'">'.$sql3['titel'].'</a>'.$mark2.'</td><td class="link">'.$mark1.'<a href="/foro.php?cat='.urlencode($sql3['kategorie']).'">'.$sql3['kategorie'].'</a>'.$mark2.'</td><td>'.$mark1.$aktualitaet.' von '.$sql3['lastposter'].$mark2.'</td></tr>';
}
?>
</tbody>
</table>
