...
<p><?php echo __('Ich möchte als %1$s mitspielen. Das Spiel ist zu 100%% kostenlos. Meine E-Mail-Adresse ist %2$s. An diese Adresse soll mir jetzt gleich ein Passwort zugeschickt werden, mit dem ich mich dann einloggen kann.', '<strong>'.htmlspecialchars($uName, ENT_QUOTES, 'UTF-8').'</strong>', '<strong>'.htmlspecialchars($uMail, ENT_QUOTES, 'UTF-8').'</strong>'); ?></p>
...
<p><input type="hidden" name="reg_benutzername" id="reg_benutzername" value="<?php echo htmlspecialchars($uName, ENT_QUOTES, 'UTF-8'); ?>" /><input type="hidden" name="reg_email" id="reg_email" value="<?php echo htmlspecialchars($uMail, ENT_QUOTES, 'UTF-8'); ?>" /><input type="submit" value="<?php echo _('Jetzt mit diesen Daten registrieren'); ?>" /></p>
