...
if ($cookie_id != CONFIG_DEMO_USER) {
	...
	// PROTOKOLL ANFANG
	$formulierung = 'Du hast dem Verein <a href="/equipo.php?id='.$team.'">'.$name.'</a> ein Angebot für ein Testspiel gemacht.';
	$sql7 = "INSERT INTO ".$prefix."protokoll (team, text, typ, zeit) VALUES ('".$cookie_team."', '".$formulierung."', 'Termine', ".time().")";
	$sql8 = mysql_query($sql7);
	$formulierung = 'Du hast eine Anfrage für ein Testspiel vom Verein <a href="/equipo.php?id='.$cookie_team.'">'.$cookie_teamname.'</a> bekommen.';
	$sql7 = "INSERT INTO ".$prefix."protokoll (team, text, typ, zeit) VALUES ('".$team."', '".$formulierung."', 'Termine', ".time().")";
	$sql8 = mysql_query($sql7);
	// PROTOKOLL ENDE
}
header('Location: /equipo.php?id='.$team.'&action=requestedFriendly');
?>
