...
if ((isset($_POST['lusername']) && isset($_POST['lpassword'])) OR $valid_bigpoint_user != '') {
    ...
    if ($valid_bigpoint_user == '') {
        ...
        if ($lologin2 && mysql_num_rows($lologin2) == 1) {
            ...
            $hashInDb = $userCandidate['password'];
            if (password_verify($lopassword, $hashInDb)) {
                ...
                if (password_needs_rehash($hashInDb, PASSWORD_DEFAULT)) {
                    ...
                    mysql_query("UPDATE ".$prefix."users SET password = '".$newHash."' WHERE ids = '".$userCandidate['ids']."'");
                }
            }
            else {
                $legacyHash = md5('1'.$lopassword.'29');
                if ($hashInDb === $legacyHash) {
                    $lologin4 = $userCandidate;
                    $newHash = mysql_real_escape_string(password_hash($lopassword, PASSWORD_DEFAULT));
                    mysql_query("UPDATE ".$prefix."users SET password = '".$newHash."' WHERE ids = '".$userCandidate['ids']."'");
                }
