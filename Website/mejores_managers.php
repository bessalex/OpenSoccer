...
if ($cookie_team != '__'.$cookie_id) {
	...
	if (mysql_num_rows($sql2) != 0) {
		...
		echo '<p style="text-align:right"><a href="/mejores_managers.php?seite='.$meinePositionsSeite.'" class="pagenava">'._('Meine Position').'</a>';
		if (mb_strlen($sql3['liga']) == 32) {
			...
			if (mysql_num_rows($ownCountry2) == 1) {
				...
				echo '<a href="/mejores_managers.php?land='.urlencode($ownCountry3['land']).'" class="pagenava">'._('Mein Land').'</a>';
			}
		}
		echo '</p>';
	}
